...
export default function TaskTrackerSection({ tasks, setTasks, thresholds, setThresholds, theme }) {
  ...
  // ── BroadcastChannel Real-Time Cross-Tab Sync (Tasks Only) ──
  const broadcastChannelRef = useRef(null);
  const isExternalUpdate = useRef(false);

  useEffect(() => {
    try {
      broadcastChannelRef.current = new BroadcastChannel('omnipulse_tasks_sync');
      ...
      <div className="glass-panel p-4 flex items-center justify-between border-l-4 border-l-blue-500">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">
            BroadcastChannel + localStorage • Multi-Tab Instant Sync
          </span>
        </div>
  ...
  const [activeChartType, setActiveChartType] = useState('line'); // 'line', 'bar', 'pie', 'linear', 'algebraic'
  const [algebraicFuncType, setAlgebraicFuncType] = useState('exponential'); // 'exponential', 'logarithmic', 'quadratic', 'linear'
  const [filterCategory, setFilterCategory] = useState('All');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-panel p-5 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs font-bold text-slate-200">
              <Target className="w-4 h-4 text-cyan-400" /> Target Threshold Setting
            </div>
        ...
        <div className={`glass-panel p-5 flex flex-col justify-between border-l-4 ${
          ...
          <div className="flex justify-between items-start">
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
              meetsGlobalThreshold ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'
            }`}>
              {meetsGlobalThreshold ? <Sparkles className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
            </div>
          </div>
          <p className="text-[11px] text-slate-400 mt-2">
            Target threshold set at <span className="text-white font-mono">{thresholds.globalTarget}%</span> across past 7 days.
          </p>
        </div>
        ...
        <div className="glass-panel p-5 flex flex-col justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-white mt-1">
              {completedCount} <span className="text-sm font-normal text-slate-400">/ {totalTasksCount} items</span>
            </div>
          </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
          <div>
            <h2 className="text-base font-bold text-white flex items-center gap-2">
              <Zap className="w-5 h-5 text-cyan-400" /> Dynamic Telemetry & Threshold Visualizer
            </h2>
            <p className="text-xs text-slate-400">
              Switch graph views (Line, Bar, Pie, Linear, Algebraic curves) to analyze task completion trends.
            </p>
          </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <h3 className="text-base font-bold text-white">Task Tracker Items</h3>
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h4 className={`text-sm font-bold ${task.completed ? 'line-through text-slate-400' : 'text-slate-100'}`}>
                        {task.title}
                      </h4>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-panel p-6 w-full max-w-md space-y-4 border border-slate-700">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              <Plus className="w-5 h-5 text-cyan-400" /> Add New Task with Threshold
            </h3>
            ...
            <form onSubmit={handleAddTask} className="space-y-4 text-xs">
              ...
              <div>
                <div className="flex justify-between text-slate-400 mb-1">
                  <span>Target Completion Threshold (%)</span>
                  <span className="font-mono text-cyan-400">{newTaskThreshold}%</span>
                </div>
                <input
                  type="range"
                  min="30"
